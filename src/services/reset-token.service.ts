...
class ResetTokenService {
  async generate(userId: string): Promise<ResetToken> {
    ...
    const resetToken: ResetToken = await prisma.resetToken.create({
      data: {
  ...
  async getUserFromToken(token: string): Promise<User> {
    const resetToken = await prisma.resetToken.findFirst({ where: { hashedToken: token } });

    if (!resetToken || resetToken.revoked) throw new ResetTokenInvalidError();

    await prisma.resetToken.update({ where: { id: resetToken.id }, data: { ...resetToken, revoked: true } });
